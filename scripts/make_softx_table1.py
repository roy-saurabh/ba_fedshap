...
Writes: <output>  (CSV)  — aggregate means ± std over 15 cells

Default paths assume you have extracted compas_raw_results.zip into
results/raw/compas/ (see README "Pre-computed raw results" section).

Usage
-----
# Zero-argument form (uses defaults):
...
def main() -> None:
    ...
    parser.add_argument(
        "--input",
        default=str(REPO_ROOT / "results" / "raw" / "compas"),
        help="Directory containing seed_*/alpha_*/full_eval.json files "
             "(default: results/raw/compas)",
    )
    parser.add_argument(
        "--output",
        default=str(REPO_ROOT / "results" / "tables" / "table1_compas_aggregate.csv"),
        help="Output CSV path (default: results/tables/table1_compas_aggregate.csv)",
    )
    ...
    if not input_dir.is_dir():
        logger.error(
            ...
            "Extract compas_raw_results.zip there first, or re-run "
            "scripts/run_softx_compas_lowcompute.sh to generate the raw results.",
            input_dir,
        )
        sys.exit(1)
    ...
    if not records:
        logger.error(
            "No full_eval.json files found in %s. "
            "Extract compas_raw_results.zip there first.",
            input_dir,
        )
        sys.exit(1)
